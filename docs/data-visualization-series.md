...
- 正式製作日期：2026/04/23。
- 實際使用工具：Canva、Gemini、ChatGPT；本作品未使用 Power BI 或 Excel。
- 使用者已提供四張完成投影片截圖；目前仍沒有案例來源清單、分析表、分鏡、時間軸或字幕安排等原始製作過程截圖。
- 公開頁面的四階段製作方法以語意化 HTML／CSS 呈現，不把完成投影片冒充原始流程證據。
...
目前 `public/media/data-visualization/` 只保留系列封面與概念圖所需的公開安全 SVG；作品一的舊流程 SVG 已由語意化 HTML／CSS 流程取代。作品二的 SVG 不含真實資料。

作品索引另使用 `work-01-index-public.svg` 與 `work-02-index-public.svg` 兩張 16:10 wrapper：前者只重用不含第三方品牌介面的既有案例主視覺，後者只重用不含原始資料或分析數值的概念化儀表板。使用者提供的完成簡報拼貼含 Spotify／第三方案例介面，權利範圍確認前不進 `public/`；使用者提供的 Power BI 真實結果 PNG 含 `user_sn` 與成績／互動分布，未取得資料提供方另行公開許可前同樣維持隔離。

作品二專屬案例內頁不把上述抽象 SVG 當作作品成果，也不載入受限截圖、影片、分析檔案或實際結果。
