...
| PA-012 | PERFORMANCE | Hero R3F chunk 與 Hero copy | 歷史單一 Three chunk 約 851 kB minified／224 kB transfer；全頁／主標隱藏式進場曾使 LCP 落在 nav，Three 也曾過早與首屏競爭。現在 lazy 3D closure 為 638232 raw／169223 gzip B，最大 chunk 483687 B | 已消除 oversized warning 與 initial preload；低階裝置的 GPU／耗電風險仍需真機證據 | Medium | current-fingerprint immutable submission artifact、17 個歷史成功 runs、逐檔 artifact／source manifests、completion markers 與 rendered behavior 已確認 | 主標／介紹首幀可見；lean R3F canvas 只 extend 必要 constructors；Three 以 visibility-aware lazy gate 載入，局部 scene boundary 保留核心文案；遞迴 built-import budget 防止 closure 或單一 chunk 回歸 | 本機 closure 已完成；真機／field evidence 待辦 | current-fingerprint：mobile Performance 95／Accessibility 97／LCP 2558 ms／TBT 37 ms；desktop 100／100／552 ms／0 ms；LCP 為 `#hero-title`、CLS 0，無 Vite oversized warning |
