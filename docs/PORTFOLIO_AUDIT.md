...
| PA-015 | GITHUB_PAGES | Actions workflow | 原本沒有部署流程；Windows-only `.ps1` scripts 不適用 Ubuntu build job | Pages 建置無法重現或直接失敗 | High | 設定檔已確認；遠端未確認 | 新增 manual-only、`windows-latest` build、Node 22／pnpm 11.7、Pages artifact deploy | 已完成（未部署） | YAML 與本機命令已檢查；因 Git 無效未能 push 或遠端執行 |
| PA-016 | RELIABILITY | Repository metadata | `.git` 是空目錄，缺少 `HEAD`、`config`、`objects`、`refs` | 無法查 branch/history、commit、push、PR 或證明演進 | Critical | 本機 filesystem 與 Git 已確認 | 從原 repository 還原 `.git` 或重新 clone；不可用 `git init` 冒充既有歷史 | 阻塞 | `git status`／`git rev-parse` 均回報 not a git repository |
