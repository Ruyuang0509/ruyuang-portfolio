...
| PA-012 | PERFORMANCE | Hero R3F chunk 與 Hero copy | 歷史單一 Three chunk 約 851 kB minified／224 kB transfer；全頁／主標隱藏式進場曾使 LCP 落在 nav，Three 也曾過早與首屏競爭。現在 lazy 3D closure 為 638232 raw／169223 gzip B，最大 chunk 483687 B | 已消除 oversized warning 與 initial preload；低階裝置的 GPU／耗電風險仍需真機證據 | Medium | current-fingerprint immutable submission artifact、20 個成功 runs、逐檔 artifact／source manifests、completion markers 與 rendered behavior 已確認 | 主標／介紹首幀可見；lean R3F canvas 只 extend 必要 constructors；Three 以 visibility-aware lazy gate 載入，局部 scene boundary 保留核心文案；遞迴 built-import budget 防止 closure 或單一 chunk 回歸 | 本機 closure 已完成；真機／field evidence 待辦 | current-fingerprint：mobile Performance 95／Accessibility 100／LCP 2557 ms／TBT 35 ms；desktop 100／100／554 ms／0 ms；LCP 為 `#hero-title` 文字、CLS 0，無 Vite oversized warning |
...
| PA-023 | PERFORMANCE_EVIDENCE | Lighthouse lineage | 本輪以目前 build inputs 重新建立 immutable submission artifact、source manifest 與 mobile／desktop raw reports | 歷史分數只解釋舊決策；最新 summary 才代表目前 source fingerprint 的 localhost lab 證據 | Medium | current source／artifact fingerprint、completion marker 與 fresh reports 已確認 | 每次 runtime、build 或 audit script 變更後重跑，不沿用舊分數 | 已完成（本機） | run `2026-07-16T22-36-38-651Z`：mobile 95／100、LCP 2557 ms、TBT 35 ms；desktop 100／100、LCP 554 ms、TBT 0；兩者 CLS 0 |
