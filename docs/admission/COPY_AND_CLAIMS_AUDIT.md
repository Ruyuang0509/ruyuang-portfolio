...
- 兩個頁面均顯示「不公開」。這項核對不等同第三方素材權利、競賽公開範圍或長期連結可用性已確認。
- `admission-evidence.js` 與 `admission-evidence.audit.js` 以三個 stable ID 對齊；submission alias 排除 audit module，但 Repository 是 public，source-only 不是 private。
- 13 條高訊號 scanner rules 與 nested bundle fixtures 已加入 source；最終為 72/72 fixtures 通過，submission 掃描涵蓋 132 files、25 text files、67 text rules 與 7 inventory rules。
...
- `pnpm run check:publication`：exit 1；11 個 Hamlet 權利／applicant-attestation blockers 仍維持阻擋。這項結果與公開文案已完成分層並不矛盾：前者判斷媒體發布權利，後者判斷公開閱讀品質與 bundle 邊界。
- 延後載入 admission 區段已在 deferred-ready／hash-settled 後同步更新 Lenis 與 ScrollTrigger layout；瀏覽器以深層錨點直接進入測試，區段載入後可重新 settle 至正確目標。
