...
目前執行環境沒有可呼叫的 `@浮士德` 代理，因此本輪依任務指定的兩個 checkpoint 進行等價內部稽核：

1. 實作前檢查主敘事、區段功能、能力誇大與招生簡章式空話。
2. 實作後逐段檢查 Pure Data 作者性、REAPER 狀態、研究構想邊界、繁體中文一致性與可核對證據。
...
| Pure Data 只是一張「學習中」卡片 | 獨立為影片、觀看指南、可證明／不能證明、AI 作者性與下一步 | 讓真實 learning artifact 可被閱讀 |
...
| 《畫本》與指定 MV 完全缺席 | 依申請者事實加入文字案例，同時明示沒有公開成片與權利核對 | 不以缺 artifact 為由刪除真實經驗，也不假裝有媒體證據 |
| 人格描述容易落入「認真、熱情、負責」 | 改用社團重整、6→17、專題角色調整與工作適應事件 | 讓特質回到具體行動 |
| 頁尾只有站內循環 | 加入真實 Portfolio 與 GitHub；研究計畫不放假下載連結 | 提供可核對出口並保持發布邊界 |
...
| Pure Data v0.2.1 在本機可操作 | 62.983 秒 MP4、H.264／AAC、抽幀與音訊檢查 | 影片含本機路徑、`validated` 與裁切標籤；只稱本機功能測試 | 修改後保留 |
...
| 《畫本》是原創短劇，角色為故事構思、攝影、剪輯 | 申請者本輪事實 | 缺成片、日期、完整 credit、活動紀錄與權利核對 | 限定後保留 |
| 《畫本》獲獎或有特定排名 | 沒有證據 | 不得推論 | 刪除 |
| 指定 MV 是申請者原創動畫／音樂 | 第三方角色、影像、音樂 | 只主張選曲、取材、篩選與剪輯 | 刪除 |
...
- `validated` 只在揭露原始介面限制時出現；網站狀態一律用「本機功能測試／尚待驗證」。
...
- 不加入《畫本》或指定 MV 影片、截圖、年份、獎項、排名或觀看數。
