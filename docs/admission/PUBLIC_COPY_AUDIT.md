...
| Draft 呈現入口 | `src/draft/DraftModeEnabled.jsx` | 依 stable ID 讀取稽核紀錄，供本機 Draft 檢查 | Submission mode 透過 alias 排除 |
...
兩個 YouTube 頁面均顯示為「不公開」，但 direct URL 可觀看。這項核對只支持「連結目前可開啟且指向相符作品」，不等同公開授權、長期可用性或第三方素材權利已完成確認。公開頁使用 canonical `youtube.com/watch` URL，不改用 scanner 明確阻擋的 `youtu.be` 短網址。
...
`tests/submission-scanner.test.mjs` 同步加入 nested lazy admission chunk 的逐條失敗 fixture，以及自然公開文案應通過的對照 fixture。2026-07-26 最終執行結果為 72/72 fixtures 通過；submission 掃描涵蓋 132 files、25 text files、67 text rules 與 7 inventory rules。

## 2026-07-26 最終驗證

- `pnpm run doctor`：exit 0。
- Submission build：467 modules；initial JS gzip 191397 B、entry 148553 B、CSS 44122 B。
- Scanner regression fixtures：72/72；submission 輸出掃描 132 files、25 text files、67 text rules、7 inventory rules。
- 針對本文件列出的公開施工／稽核高訊號字串另做 bundle 獨立掃描，結果為 0 hits。
- `pnpm run check:publication`：exit 1；11 個 Hamlet 權利／applicant-attestation blockers 維持阻擋，未被文案分層、scanner 或 submission build 降級。
- 延後載入區段已補上 `portfolio:deferred-ready`／`portfolio:hash-settled` 後的 Lenis 與 ScrollTrigger layout refresh；瀏覽器實測確認直接開啟深層錨點時，內容載入後會重新定位到目標區段。

## 發布判讀

- Public copy 通過作品敘事檢查，不代表作品權利已清除。
- Submission bundle 排除 audit module，不代表 public Repository 中的 audit source 是私密資料。
- YouTube direct URL 可播放，不代表取得下載、重製、嵌入或第三方素材再授權。
- Hamlet media 的 publication gate 維持獨立判讀；本輪公開文案整理不會降低或繞過既有 blockers。
